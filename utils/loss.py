...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = torch.tensor([.26, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89], device=device) / 10.0
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            gain = torch.ones(33, device=targets.device).long()  # normalized to gridspace gain
        else:
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                gain[2:32] = torch.tensor(p[i].shape)[15*[3, 2]]  # xyxy gain
            else:
